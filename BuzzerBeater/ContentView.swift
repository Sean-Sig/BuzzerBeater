      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 28) {
          ...
          if !store.liveGames.isEmpty {
            SectionHeader(title: "Live Now", systemImage: "dot.radiowaves.left.and.right", tint: .red)
            VStack(spacing: 14) {
          ...
          if !store.upcomingGames.isEmpty {
            SectionHeader(title: "Upcoming", systemImage: "calendar", tint: .blue)
              .padding(.horizontal)
          ...
          if !store.pastGames.isEmpty {
            SectionHeader(title: "Results", systemImage: "flag.checkered", tint: .secondary)
              .padding(.horizontal)
            ScrollView(.horizontal, showsIndicators: false) {
              HStack(spacing: 14) {
...
struct LiveGameCard: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      HStack(spacing: 0) {
        TeamScoreColumn(team: game.awayTeam, isWinning: game.awayTeam.score > game.homeTeam.score)
        VStack(spacing: 4) {
          LiveBadge()
          Text(game.quarter)
        ...
        .frame(width: 70)
        TeamScoreColumn(team: game.homeTeam, isWinning: game.homeTeam.score > game.awayTeam.score)
      }
      .padding(.vertical, 16)
...
struct TeamScoreColumn: View {
  ...
  var body: some View {
    VStack(spacing: 6) {
      ...
      Text(team.name.components(separatedBy: " ").last ?? team.abbreviation)
...
struct PastGameCard: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      ...
      HStack(spacing: 12) {
        FinalTeamRow(team: game.awayTeam, isWinner: game.awayTeam.score > game.homeTeam.score)
        Spacer()
        Text("FINAL")
          .font(.caption2.weight(.bold))
          .foregroundStyle(.secondary)
        Spacer()
        FinalTeamRow(team: game.homeTeam, isWinner: game.homeTeam.score > game.awayTeam.score)
      }
